# -*- coding: utf-8 -*-
"""
LGO_v2_2.py — DEAP engine (v2.1 core) with v3-style compatibility
==================================================================

Version: 2.2.0
Date: Dec 8, 2025

This file is a fork of LGO_v2_1.py used in the original paper.
v3.x is the future work.

Changes vs v2.1
---------------
1. The core evolutionary search `run_lgo_sr_v2` is **unchanged**.
2. Add small compatibility helpers so that v3.x runners / comparison
   scripts can import it:
   - `run_lgo_sr_v3` is an alias of `run_lgo_sr_v2`.
   - `create_primitive_set(...)` is provided as a no-op shim.
"""
...
# =============================================================================
# v2.2 backward‑compatibility helpers
# =============================================================================

# 未来 LGO v3.x 的脚本预期有一个叫 run_lgo_sr_v3 的入口函数。
# 在 v2.2 中我们直接把它指向原来的 v2 引擎，以免改动上层代码。
run_lgo_sr_v3 = run_lgo_sr_v2


def create_primitive_set(*args, **kwargs):
    """
    Compatibility shim for old comparison scripts that used::

        from LGO_v3_x import create_primitive_set

    v2.x 引擎内部通过 `build_typed_pset` 自行构造 PrimitiveSet，
    上层并不需要显式调用这个函数。这里保留一个空壳只是为了
    避免 ImportError。
    """
    import warnings
    warnings.warn(
        "create_primitive_set() is a no-op shim in LGO_v2_2. "
        "The primitive set is constructed inside run_lgo_sr_v2.",
        RuntimeWarning,
    )
    return None