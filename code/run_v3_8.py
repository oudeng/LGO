...
run_v3_8.py
-----------
Due to LGO_v2 to v2.1, upgrade run_v3.7 to 3.8.
"""
...
# ------------------------ Engines ------------------------
try:
    from src.LGO_v2_1 import run_lgo_sr_v2
except Exception:
    run_lgo_sr_v2 = None
try:
    from src.PySR_v1 import run_pysr_sr_v1
except Exception:
    run_pysr_sr_v1 = None
try:
    from src.Operon_v1 import run_operon_sr_v1
except Exception:
    run_operon_sr_v1 = None

try:
    from src.PSTree_v2_2 import run_pstree_once as _pst_run
    from src.PSTree_v2_2 import PSTreeConfig as _PSTCfg
except Exception:
    _pst_run, _PSTCfg = None, None
try:
    from code.baselines.RILS_ROLS_v2_1 import run_rils_rols_once as _rils_run
    from code.baselines.RILS_ROLS_v2_1 import RILSROLSConfig as _RILSCfg
except Exception:
    _rils_run, _RILSCfg = None, None

ENGINES = {"lgo": run_lgo_sr_v2, "pysr": run_pysr_sr_v1, "operon": run_operon_sr_v1,
           "pstree": _pst_run, "rils_rols": _rils_run}

# ---------------------- Engine wrappers ----------------------
def _call_lgo(X, y, feature_names, experiment, hparams):
    if run_lgo_sr_v2 is None:
    ...
    sig = inspect.signature(run_lgo_sr_v2).parameters
