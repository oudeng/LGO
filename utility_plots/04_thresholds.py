# -*- coding: utf-8 -*-
"""
04_thresholds_v3_8.py – Enhanced threshold audit v3.7 (publication-ready)
...
def _plot_agreement_heatmap(df: pd.DataFrame, dataset: str, outdir: str, 
    ...
    path = os.path.join(outdir, f"04_agreement_{dataset}.png")
...
def _plot_coverage_heatmap(df: pd.DataFrame, dataset: str, outdir: str,
    ...
    plt.tight_layout()
    path = os.path.join(outdir, f"04_coverage_{dataset}.png")
    fig.savefig(path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return path
...
def _plot_error_distribution(df: pd.DataFrame, dataset: str, outdir: str,
    ...
    path = os.path.join(outdir, f"04_error_dist_{dataset}.png")
    fig.savefig(path, dpi=dpi, bbox_inches="tight")
    plt.close(fig)
    return path
...
def process_dataset(dataset: str, df_units: pd.DataFrame, df_audit: pd.DataFrame,
    ...
    if not A.empty:
        for _, row in A.iterrows():
            ...
            # FIXED v3.8: only_anchored requires BOTH guideline AND median
    ...
    # Add features from units that aren't in audit
        ...
        for _, row in U.iterrows():
            ...
            # FIXED v3.8: Better quality score with proper fallback
...
def main():
    ...
    if all_results:
        ...
        summary_path = os.path.join(args.outdir, "04_thresholds_summary.csv")
